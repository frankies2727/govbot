...
//
// TO UPDATE THIS FILTER FOR wy-legislation (Wyoming):
// 1. First, gather real data by running this command:
//    `just govbot source --repos=wy --limit=100`
...
// 5. Test your changes: `just govbot source --repos=wy --limit=100 --filter=default`
//
// Current filter removes: routine introductions, referrals, and status updates
// ======================================

// Filter for wy-legislation (Wyoming)
        ...
        if let Some(action) = log.get("action") {
            ...
            if let Some(description) = action.get("description") {
                if let Some(desc_str) = description.as_str() {
                    // Filter out "H Introduced and Referred to" - routine introduction + referral
                    if desc_str.starts_with("H Introduced and Referred to") {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "S Introduced and Referred to" - routine introduction + referral
                    if desc_str.starts_with("S Introduced and Referred to") {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "Bill Number Assigned" - routine status
                    if desc_str == "Bill Number Assigned" {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "H Placed on General File" - routine status
                    if desc_str == "H Placed on General File" {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "S Placed on General File" - routine status
                    if desc_str == "S Placed on General File" {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "S President Signed" - routine status
                    if desc_str.starts_with("S President Signed") {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "Pursuant to JR" - routine status
                    if desc_str.starts_with("Pursuant to JR") {
                        return FilterResult::FilterOut;
                    }
                }
            }

...
//
// TO UPDATE THIS FILTER FOR ri-legislation (Rhode Island):
// 1. First, gather real data by running this command:
//    `just govbot source --repos=ri --limit=100`
...
// 5. Test your changes: `just govbot source --repos=ri --limit=100 --filter=default`
//
// Current filter removes: routine introductions, referrals, and scheduling
// ======================================

// Filter for ri-legislation (Rhode Island)
// Filters out routine introductions, referrals, and scheduling
        ...
        if let Some(action) = log.get("action") {
            ...
            if let Some(description) = action.get("description") {
                if let Some(desc_str) = description.as_str() {
                    // Filter out "Introduced, referred to" - routine introduction + referral
                    if desc_str.starts_with("Introduced, referred to") {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "Placed on House Calendar" - routine calendar scheduling
                    if desc_str.starts_with("Placed on House Calendar") {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "Placed on Senate Calendar" - routine calendar scheduling
                    if desc_str.starts_with("Placed on Senate Calendar") {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "Scheduled for hearing and/or consideration" - routine scheduling
                    if desc_str.starts_with("Scheduled for hearing and/or consideration") {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "Scheduled for consideration" - routine scheduling
                    if desc_str.starts_with("Scheduled for consideration") {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "Proposed Substitute" - routine substitution
                    if desc_str == "Proposed Substitute" {
                        return FilterResult::FilterOut;
                    }
                }
            }

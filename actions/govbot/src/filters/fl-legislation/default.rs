...
//
// TO UPDATE THIS FILTER FOR fl-legislation (Florida):
// 1. First, gather real data by running this command:
//    `just govbot source --repos=fl --limit=100`
...
// 5. Test your changes: `just govbot source --repos=fl --limit=100 --filter=default`
//
// Current filter removes: routine filings, referrals, and committee status updates
// ======================================

// Filter for fl-legislation (Florida)
// Filters out routine filings, referrals, and committee status updates
        ...
        if let Some(action) = log.get("action") {
            ...
            if let Some(description) = action.get("description") {
                if let Some(desc_str) = description.as_str() {
                    ...
                    // Filter out "Now in..." - routine committee status updates
                    if desc_str.starts_with("Now in") {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "On Committee agenda..." - routine scheduling
                    if desc_str.starts_with("On Committee agenda") {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "Added to...Subcommittee agenda" - routine scheduling
                    if desc_str.contains("Added to") && desc_str.contains("agenda") {
                        return FilterResult::FilterOut;
                    }
                }
            }

...
//
// TO UPDATE THIS FILTER FOR nd-legislation (North Dakota):
// 1. First, gather real data by running this command:
//    `just govbot source --repos=nd --limit=100`
...
// 5. Test your changes: `just govbot source --repos=nd --limit=100 --filter=default`
//
// Current filter removes: routine introductions, referrals, and committee hearings
// ======================================

// Filter for nd-legislation (North Dakota)
// Filters out routine introductions, referrals, and committee hearings
        ...
        if let Some(action) = log.get("action") {
            ...
            if let Some(description) = action.get("description") {
                if let Some(desc_str) = description.as_str() {
                    // Filter out "Introduced, first reading, referred" - routine introduction + referral
                    if desc_str.starts_with("Introduced, first reading, referred") {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "Committee Hearing" - routine hearing scheduling
                    if desc_str.starts_with("Committee Hearing") {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "Reported back, do pass" - routine committee report
                    if desc_str.starts_with("Reported back, do pass") {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "Received from Senate" - routine status
                    if desc_str == "Received from Senate" {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "Signed by President" - routine status
                    if desc_str == "Signed by President" {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "Signed by Speaker" - routine status
                    if desc_str == "Signed by Speaker" {
                        return FilterResult::FilterOut;
                    }
                }
            }

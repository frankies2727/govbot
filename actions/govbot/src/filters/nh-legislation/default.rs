...
//
// TO UPDATE THIS FILTER FOR nh-legislation (New Hampshire):
// 1. First, gather real data by running this command:
//    `just govbot source --repos=nh --limit=100`
...
// 5. Test your changes: `just govbot source --repos=nh --limit=100 --filter=default`
//
// Current filter removes: routine introductions, referrals, and hearing scheduling
// ======================================

// Filter for nh-legislation (New Hampshire)
// Filters out routine introductions, referrals, and hearing scheduling
        ...
        if let Some(action) = log.get("action") {
            ...
            if let Some(description) = action.get("description") {
                if let Some(desc_str) = description.as_str() {
                    // Filter out "Introduced...and referred to" - routine introduction + referral
                    if desc_str.contains("Introduced") && desc_str.contains("referred to") {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "Public Hearing:" - routine hearing scheduling
                    if desc_str.starts_with("Public Hearing:") {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "Executive Session:" - routine scheduling
                    if desc_str.starts_with("Executive Session:") {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "Full Committee Work Session:" - routine scheduling
                    if desc_str.starts_with("Full Committee Work Session:") {
                        return FilterResult::FilterOut;
                    }
                }
            }

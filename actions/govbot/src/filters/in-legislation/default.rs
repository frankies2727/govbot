...
//
// TO UPDATE THIS FILTER FOR in-legislation (Indiana):
// 1. First, gather real data by running this command:
//    `just govbot source --repos=in --limit=100`
...
// 5. Test your changes: `just govbot source --repos=in --limit=100 --filter=default`
//
// Current filter removes: routine first readings, referrals, and authorship notifications
// ======================================

// Filter for in-legislation (Indiana)
// Filters out routine first readings, referrals, and authorship notifications
...
pub fn should_keep(entry: &Value) -> FilterResult {
    if let Some(log) = entry.get("log") {
        if let Some(action) = log.get("action") {
            // Filter out "reading-1" and "referral-committee" classifications
                ...
                if let Some(class_array) = classification.as_array() {
                    for class in class_array {
                        if let Some(class_str) = class.as_str() {
                            if class_str == "reading-1" || class_str == "referral-committee" {
            ...
            if let Some(description) = action.get("description") {
                if let Some(desc_str) = description.as_str() {
                    // Filter out "First reading: referred to" - routine referral
                    if desc_str.starts_with("First reading: referred to") {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "Authored by" - routine authorship notification
                    if desc_str.starts_with("Authored by") {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "Coauthored by" - routine co-authorship notification
                    if desc_str.starts_with("Coauthored by") {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "House sponsor:" - routine sponsorship notification
                    if desc_str.starts_with("House sponsor:") {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "Cosponsor:" - routine sponsorship notification
                    if desc_str.starts_with("Cosponsor:") {
                        return FilterResult::FilterOut;
                    }
                }
            }

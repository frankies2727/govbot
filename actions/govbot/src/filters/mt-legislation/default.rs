...
//
// TO UPDATE THIS FILTER FOR mt-legislation (Montana):
// 1. First, gather real data by running this command:
//    `just govbot source --repos=mt --limit=100`
...
// 5. Test your changes: `just govbot source --repos=mt --limit=100 --filter=default`
//
// Current filter removes: routine hearings, scheduling, and draft status updates
// ======================================

// Filter for mt-legislation (Montana)
// Filters out routine hearings, scheduling, and draft status updates
        ...
        if let Some(action) = log.get("action") {
            ...
            if let Some(description) = action.get("description") {
                if let Some(desc_str) = description.as_str() {
                    // Filter out "(H) Hearing -" and "(S) Hearing -" - routine hearing notifications
                    if desc_str.starts_with("(H) Hearing -") || desc_str.starts_with("(S) Hearing -") {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "Scheduled for 2nd Reading" - routine scheduling
                    if desc_str.contains("Scheduled for 2nd Reading") {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "(LC) Draft Ready for Delivery" - routine draft status
                    if desc_str == "(LC) Draft Ready for Delivery" {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "(LC) Drafter Assigned" - routine draft status
                    if desc_str == "(LC) Drafter Assigned" {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "(LC) Draft On Hold" - routine draft status
                    if desc_str == "(LC) Draft On Hold" {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "(LC) Draft Died in Process" - routine draft status
                    if desc_str == "(LC) Draft Died in Process" {
                        return FilterResult::FilterOut;
                    }
                    // Filter out "(H) Rereferred to Committee -" - routine re-referral
                    if desc_str.starts_with("(H) Rereferred to Committee -") {
                        return FilterResult::FilterOut;
                    }
                }
            }
